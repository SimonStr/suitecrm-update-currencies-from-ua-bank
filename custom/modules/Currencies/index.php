...
if ($current_user->is_admin) {
    ...
    if (isset($_REQUEST["percent"])) {
        ...
        $GLOBALS['db']->query("UPDATE currencies SET percent = '{$percent}' WHERE id = '{$id}'");
    }
    $merge_button = '';
    $pretable = '';
...
EOQ;
    ...
    $query_percent = $db->query("SELECT percent FROM currencies WHERE id = '{$focus->id}'");
    $focus->percent = $db->fetchByAssoc($query_percent)["percent"];
    ...
    $sugar_smarty->assign('PERCENT', $focus->percent);
    $sugar_smarty->assign('CURRENCY_ISO4217', $lc->list[0]->iso4217);
